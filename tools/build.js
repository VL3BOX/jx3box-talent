...
(async () => {
    ...
    const datas = {
        ...
        skill: [],
        skill_txt: (
            await readTabFile(path.join(__dirname, "../raw/skill.txt"))
        ).reduce((acc, cur) => {
        ...
        skill_tab: (
            await readTabFile(path.join(__dirname, "../raw/skills.tab"))
        ).reduce((acc, cur) => {
            acc[cur.SkillID] = cur;
            return acc;
        }, {}),
        buff: [],
        buff_txt: (
            await readTabFile(path.join(__dirname, "../raw/buff.txt"))
        ).reduce((acc, cur) => {
        ...
        buff_tab: (
            await readTabFile(path.join(__dirname, "../raw/buff.tab"))
        ).reduce((acc, cur) => {
    ...
    Logger.info("合并需要用到的 skill 数据......");
    {
        let bar = await buildProgressBar(
            Object.keys(datas.skill_txt).length,
            "合并需要用到的 skill 数据"
        );
        ...
        let remain_skills = Object.values(datas.skill_tab).filter(
            t => !t.__scan
        );
    ...
    {
        let bar = await buildProgressBar(
            Object.keys(datas.buff_tab).length,
            "buff合表中...",
            "..."
        );
        ...
        let remain_buff = Object.values(datas.buff_txt).filter(
            buff => !buff.__scan
        );
    ...
    fs.writeFileSync(
        `./output/v${dateFormat(new Date())}.json`,
